...
class LocateWorkflows:
    ...
    def find(self):
        ...
        if UnitTestUtilities.in_unit_test(self):
            print("in unit test")
            currrent_search_roots = [os.path.dirname(__file__),]
        else:
            print("not in unit test")
            currrent_search_roots = search_roots[Platform.get_current_platform()]
        search_names = ["EnergyPlus*", "energyplus*", "EP*", "ep*", "E+*", "e+*"]
        found_directories = set()
        for search_root in currrent_search_roots:
